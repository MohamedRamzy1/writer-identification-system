...
class LPQFeatureExtractor:
    ...
    An implementation of LPQ texture descriptor.
    ...
        ...
        for start_x in range(img.shape[0]):
            for start_y in range(img.shape[1]):
                ...
                aggregated_rsp = np.array([
                    rsp1.real, rsp2.real, rsp3.real, rsp4.real, rsp1.imag,
                    rsp2.imag, rsp3.imag, rsp4.imag
                ])
    ...
    def fit(self, lines):
        # loop over each lines and extrct LPQ features
        lqp_features = []
        for line in lines:
